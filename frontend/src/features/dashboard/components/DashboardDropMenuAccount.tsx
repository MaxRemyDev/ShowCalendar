		...
		<DropdownMenu>
			<DropdownMenuTrigger asChild>
				<Button variant="secondary" size="icon" className="rounded-full">
					<Avatar>
						<AvatarImage src={fakeUserData.avatarUrl} alt={`@${fakeUserData.name}`} />
						<AvatarFallback>
							{fakeUserData.name.slice(0, 2).toUpperCase()}
						</AvatarFallback>
					</Avatar>
			...
			<DropdownMenuContent align="end" className="w-56 p-4 rounded-xl">
				<DropdownMenuLabel>
					<span>{fakeUserData.name}</span>{" "}
					<span className="font-light">{fakeUserData.email}</span>
				</DropdownMenuLabel>
				<DropdownMenuSeparator />
				<DropdownMenuGroup>
